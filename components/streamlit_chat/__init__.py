...
def message(message: str, 
    ...
    """
    if logo:
        return _streamlit_chat(message=message, seed=seed, isUser=is_user, logo=logo, key=key)
    else:
        if not avatar_style:
            avatar_style = "fun-emoji" if is_user else "bottts"
        return _streamlit_chat(message=message, seed=seed, isUser=is_user, avatarStyle=avatar_style, key=key)
...
if not _RELEASE:
    ...
    """
    feedback = message("Hello, I am a Chatbot, how may I help you?")
    if feedback:
        print(feedback)
